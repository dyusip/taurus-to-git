...
@section('index-content')
    ...
    z-index: 9999; /* Specify a stack order in case you're using a different order for other elements */ padding: 23%; ">
            ...
            <div class="wrapper wrapper-content animated fadeInRight">
                <form method="post" name="myform" id="myform" action="/miscellaneous">
                    {{csrf_field()}}
                    <input type="hidden" name="msh_code" id="msh_code" value="{{ $num }}">
                    {{--<input type="hidden" name="msh_branch_code" id="msh_branch_code" value="{{ Auth::user()->branch }}">--}}
                    <input type="hidden" name="msh_prep_by" id="msh_prep_by" value="{{ Auth::user()->username }}">
                    <div class="row">
                        <div class="col-lg-7">
                                    ...
                                    <div class="row">
                                        <div class="col-sm-7 b-r">
                                            <div class="form-group">
                                                ...
                                                <select name="msh_branch_code" id="branch_code" class="form-control branch_select select2_demo_1" required>
                                                    <option value=""></option>
                                        ...
                                        <div class="col-sm-7 b-r">
                                            <div class="form-group">
                                                ...
                                                <div class='col-md-12 col-sm-12 col-xs-12 input-group date' id='datepicker'>
                                                    <input readonly placeholder="" type="text" id="reqDate" name="msh_date"
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-striped table-bordered table-hover" id="dataTables-salesOrder" data-page-size="15">
                                            ...
                                            <tr>
                                                ...
                                                <th data-hide="phone" width="6%">Misc Qty</th>
                                                <th data-hide="phone,tablet" width="7%">Cost</th>
                                                <th data-hide="phone,tablet" width="7%">Remarks</th>
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <div class="btn-group ">
                                                ...
                                                <button type="submit" class="btn btn-primary ladda-button ladda-button-demo btn-sm" data-style="zoom-in"><i class="fa fa-check-circle"></i> Create Miscellaneous</button>
                                            </div>
                                        ...
                                        <div class="col-md-6 text-right">
                                            ...
                                            <div class="form-inline">
                                                ...
                                                <input class="form-control text-right" readonly id="totalAmount" name="msh_total" type="text">
                                            </div>
...
<script>
    ...
    $('tbody').delegate('#qty','keyup',function () {
        ...
        if(Number(qty) > Number(available)){
            tr.find('#remarks').val('IN');
            amount = (qty - available) * price;
            tr.find('#misc_qty').val(qty - available);
            tr.find('#amount').val(amount);
            totalAmount();
        }else if(Number(qty) < Number(available)){
            tr.find('#remarks').val('OUT');
            amount = (available - qty) * price;
            tr.find('#misc_qty').val(available - qty);
            tr.find('#amount').val(amount);
            totalAmount();
        }else if(Number(qty) == Number(available)){
            toastr.error("Quantity adjustment should not equal to available");
            tr.find('#amount').val(0);
            tr.find('#qty').val('');
            tr.find('#misc_qty').val('');
            totalAmount();
        }
    });
    ...
    function products() {
        ...
        var count = $('#count').val();
        $.ajax({
            url: "miscellaneous/create/"+branch,
            success: function (data) {
    ...
    function addRow() {
        var row = "<tr> " +
            ...
            '<td><input type="text" name="misc_qty[]" required id="misc_qty" class="form-control" readonly></td> ' +
            '<td><input type="text" name="cost[]" id="cost" class="form-control" readonly></td> ' +
            '<td><input type="text" name="remarks[]" id="remarks" class="form-control" readonly></td> ' +
    ...
    $('tbody').delegate('#product','change',function () {
        ...
        $.ajax({url: "miscellaneous/"+item+"/"+branch,
            beforeSend: function() {
                $('#main-spinner').fadeIn();
            },
            success: function(data) {
                ...
                if(Number(tr.find('#qty').val()) > Number(data.quantity)){
                    tr.find('#remarks').val('IN');
                    amount = (tr.find('#qty').val() - data.quantity) * data.cost;
                    tr.find('#misc_qty').val(tr.find('#qty').val() - data.quantity);
                    tr.find('#amount').val(amount);
                }else if(Number(tr.find('#qty').val()) < Number(data.quantity)){
                    tr.find('#remarks').val('OUT');
                    amount = (data.quantity - tr.find('#qty').val()) * data.quantity;
                    tr.find('#misc_qty').val(data.quantity - tr.find('#qty').val());
                    tr.find('#amount').val(amount);
                }else if(Number(tr.find('#qty').val()) == Number(data.quantity)){
                    tr.find('#amount').val(0);
                    tr.find('#qty').val('');
