...
class NotifyPOController extends Controller
{
    //
    public function po_index(){
        $pos = PoHeader::where(['status' => 'AP'])->get();
        return view('Partsman.view_po.po',compact('pos'));
    }
    ...
    public function printPO(Request $request)
    {
        ...
        $po = PoHeader::where(['po_code'=>$request->PONo])->where(function ($query){
        ...
        foreach ($po->po_detail as $product) {
            ...
            $fpdf->Row1(array($product->prod_code,$product->prod_name,$product->prod_qty,$product->prod_uom,Number_Format($product->prod_price,2),$product->prod_less."%",Number_Format($product->prod_amount,2)));
        }
