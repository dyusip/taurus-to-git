...
class TransferPrintController extends Controller
{
    ...
    {
        return view($this->position().'.print_transfer.transfer');
    }
    public function show($id)
    {
        /*$transfer = TransferHeaders::where('transfer_details','transfer_details.td_code','=','transfer_headers.tf_code')
            ...
            ->select('*');*/
        $transfer = TransferHeaders::where(['tf_status' => 'AP']) ->with('tf_fr_branch')->with('tf_to_branch')->orderBy('tf_date', 'desc')->orderBy('tf_code', 'desc')
            ->with('tf_prep_by')->selectRaw('distinct transfer_headers.*');

        return Datatables::of($transfer) ->addColumn('action', function ($transfer) {
            return new HtmlString('<a href="/transferred/print/pdf/'.$transfer->tf_code.'" data-toggle="tooltip" target="_blank" title="Print" class="text-success" id="btn-edit" data-id='.$transfer->tf_code.'><i class="fa fa-print"></i></a>');
        })->make();
    }
